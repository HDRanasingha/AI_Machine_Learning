...
for userIdx = 1:numUsers
    ...
    fileName = sprintf('U%02d_Acc_TimeD_MDay.mat', userIdx);
    multiDayData = load(fileName);

    % Display the variables 
    fprintf('Variables in %s:\n', fileName);
    disp(fieldnames(multiDayData));

    
    if isfield(multiDayData, 'Acc_TD_Feat_Vec')
        userData = multiDayData.Acc_TD_Feat_Vec; 
    else
        error('Variable "Acc_TD_Feat_Vec" not found in %s', fileName);
    end
