...
labelsFDay = ones(size(Acc_TimeD_FDay, 1), 1);  % Label 1 for first-day data
labelsMDay = zeros(size(Acc_TimeD_MDay, 1), 1);  % Label 0 for multi-day data
combinedLabels = [labelsFDay; labelsMDay];  % Combine the labels

% PCA for feature selection
[coeff, score, latent] = pca(combinedData);
explainedVariance = cumsum(latent) / sum(latent);
numComponents = find(explainedVariance >= 0.95, 1); % 95% variance
