...
for userIdx = 1:numberofUsers
    ...
    for fileIdx = 1:numberofFilesPerUser
        ...
        if isfield(data, 'Acc_FD_Feat_Vec')
            Acc_FreqD_FDay = [Acc_FreqD_FDay; dataFuDay.Acc_FD_Feat_Vec];
        ...
        elseif isfield(data, 'TDFD_Feat_Vec')
            ...
            warning('Unknown variable in file %s', fileNames{fileIdx});
        end
    end
end
...
trainDataFDay = Acc_FreqD_FDay; % Training/validation data
testDataMDay = Acc_FreqD_MDay;  % Testing data
...
historyFrequencyModalFile = 'historyFrequencyModal.mat';

% Check if the file exists
if isfile(historyFrequencyModalFile)
    % Load previous accuracy data
    load(historyFrequencyModalFile, 'historyFrequencyModal');
else
    % Initialize an empty array for storing accuracies
    historyFrequencyModal = [];
end
...
if ~isempty(Acc_FreqD_FDay)
    ...
    historyFrequencyModal = [historyFrequencyModal; accuracyPercentage];
    save(historyFrequencyModalFile, 'historyFrequencyModal');
    ...
    figure;
    plot(historyFrequencyModal, '-o', 'LineWidth', 2, 'MarkerSize', 6);
    xlabel('Run Number');
    ylabel('Accuracy (%)');
    title('Frequncy Accuracy');
    ylim([0 100]);
    grid on;
end
