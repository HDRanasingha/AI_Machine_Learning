...
trainDataFDay = Acc_TimeD_FDay; % Training/validation data
testDataMDay = Acc_TimeD_MDay;  % Testing data
...
historyTimeModalFile = 'historyTimeModal.mat';

% Check if the file exists
if isfile(historyTimeModalFile)
    % Load previous accuracy data
    load(historyTimeModalFile, 'historyTimeModal');
else
...
if ~isempty(Acc_TimeD_FDay)
    % Split the dataset
    [trainData, valData, testData] = splitData(Acc_TimeD_FDay, 0.8, 0.1);
    ...
    historyTimeModal = [historyTimeModal; accuracyPercentage];
    save(historyTimeModalFile, 'historyTimeModal');
    ...
    figure;
    plot(historyTimeModal, '-o', 'LineWidth', 2, 'MarkerSize', 6);
    xlabel('Run Number');
    ylabel('Accuracy (%)');
    title('Time Accuracy ');
    ylim([0 100]);
